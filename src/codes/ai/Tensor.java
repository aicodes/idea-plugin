...
/**
class TensorQuiver {
	ExecutorService tensorFetchService = Executors.newFixedThreadPool(16);
	ConcurrentHashMap<String, Tensor> tensorCache;

	public void fetchTensor(String name) {
		Future<Tensor> future = tensorFetchService.submit((Callable<Tensor>) () -> {

			// Add to gateway hashmap.
			// Fetch things
			// Save back to cache
			// Remove from HashMap
		});
	...
	Tensor getTensor(PsiMethod method) {
		String name = method.getName();
		return getTensor(name);
	}

	Tensor getTensor(String name) {
		if (tensorCache.containsKey(name)) {
			return tensorCache.get(name);
		} else {
			fetchTensor(name);
			return Tensor.EMPTY;
		}
